    ...
    01  WS-SEARCH-RESULT-IDX      PIC 9(4) VALUE 0.
      ...
      *>     PERFORM DISPLAY-AND-LOG
       ...
       DISPLAY-PROFILE-BY-ID.
           ...
           MOVE SPACES TO WS-MSG
           STRING "Name: " WS-PROF-FIRST(WS-I) DELIMITED BY SIZE " " DELIMITED BY SIZE WS-PROF-LAST(WS-I) DELIMITED BY SIZE INTO WS-MSG END-STRING
           PERFORM DISPLAY-AND-LOG
           MOVE SPACES TO WS-MSG
           ...
           PERFORM DISPLAY-AND-LOG
           MOVE SPACES TO WS-MSG
           STRING "Graduation Year: " WS-PROF-GYEAR(WS-I) DELIMITED BY SIZE INTO WS-MSG END-STRING
           PERFORM DISPLAY-AND-LOG
           MOVE SPACES TO WS-MSG
           STRING "About Me: " WS-PROF-ABOUT(WS-I) DELIMITED BY SIZE INTO WS-MSG END-STRING
           PERFORM DISPLAY-AND-LOG
           MOVE SPACES TO WS-MSG
           STRING "Experience:" INTO WS-MSG END-STRING
           PERFORM DISPLAY-AND-LOG
           MOVE WS-PROF-EXPERIENCES(WS-I) TO WS-T1
           PERFORM FORMAT-AND-DISPLAY-BLOCK
           MOVE SPACES TO WS-MSG
           STRING "Education:" INTO WS-MSG END-STRING
           PERFORM DISPLAY-AND-LOG
           MOVE WS-PROF-EDUCATIONS(WS-I) TO WS-T1
           PERFORM FORMAT-AND-DISPLAY-BLOCK
           EXIT PARAGRAPH.
       FORMAT-AND-DISPLAY-BLOCK.
           *> Display each line of WS-T1 with indentation, preserving leading spaces
           MOVE 1 TO WS-TMP-COUNT
           PERFORM UNTIL WS-TMP-COUNT > FUNCTION LENGTH(WS-T1)
               MOVE SPACES TO WS-MSG
               STRING "  " DELIMITED BY SIZE INTO WS-MSG END-STRING
               STRING WS-T1(WS-TMP-COUNT:80) DELIMITED BY SIZE INTO WS-MSG(3:) END-STRING
               PERFORM DISPLAY-AND-LOG
               ADD 80 TO WS-TMP-COUNT
           END-PERFORM
           EXIT PARAGRAPH.
