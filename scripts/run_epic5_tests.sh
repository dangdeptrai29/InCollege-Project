#!/usr/bin/env bash
# This script runs Epic 5 tests for the InCollege program with a two-phase approach:
...
# For Epic 5 tests, we always start with empty database
reset_data() {
    ...
    # Create empty files for Epic 5 tests (no backup restoration)
    ...
    if [ $part_count -eq 0 ]; then
        ...
        for part_file in "$setup_temp_dir"/part_*.txt; do
            if [ -s "$part_file" ]; then
                ...
                # Run the program
                ./incollege > "$TEMP_DIR/setup_output_${test_case_num}_part_${part_num}.txt" 2>&1
...
# For Epic 5, we start with clean database - clear any existing backup to force clean state
echo -e "${BLUE}Epic 5 tests will use clean/empty database (no existing users)${NC}"
