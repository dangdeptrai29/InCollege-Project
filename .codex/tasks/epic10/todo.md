...
- [x] Rework registration/login validation (password rules, account limit, login loop)
...
- [x] Fix profile create/edit logic (required fields, saving/loading optional data)
...
- [x] Correct search logic (exact match, record retrieval, profile lookup)
...
- [x] Verify persistence files (accounts, profiles, search history) save/load consistently
...
- [x] Stabilize prompt flow/state transitions for all menus
...
- [x] Log deeper logic bugs that surface during fixes in Jira queue
  - Tests: Review logs/observations and ensure each identified issue is captured

## Part 2 — Quality-of-Life Improvements
- [x] Improve validation messaging clarity based on logic outcomes
...
- [x] Add flow control options (returns/back navigation, no dead ends)
...
- [x] Simplify logic paths for login/profile/search modules
...
- [x] Handle edge cases (empty profiles, missing files, invalid inputs)
...
- [x] Resolve QOL issues tied to persistence (atomic writes, data integrity)
...
- [x] Refactor core logic modules (login, profile, search) for readability
...
- [x] Remove duplicated conditionals and improve variable naming
...
- [x] Clean up file I/O routines for accounts/profiles/search data
...
- [x] Document new structure and logic decisions as docstrings/comments
...
- [x] Capture summary of completed work and remaining risks
    - Summary: Refreshed login/account/profile/search flows (validation, persistence, back navigation), added search logging, and refactored menus; remaining risk: limited automated coverage beyond login suite.
- [x] Note technical debt/follow-up Jira tickets needed
    - Tech debt: need UI team to align display spacing with new prompts; consider converting CRLF scripts to Unix line endings.
- [x] Record dependency impacts or required coordination with Dev 2
    - Coordination: Dev 2 should mirror the new BACK instructions/echoed choices in display text tests to keep console/file expectations aligned.
